...
After creating the PRD file, set up the folder structure for each agent.

The basic folder structure for this repo is already created for you:

```
├── agency.py               # Main entry point
├── main.py                 # FastAPI integration (do not modify)
├── skill_creator/
│   ├── __init__.py
│   ├── skill_creator.py    # Agent definition
│   ├── instructions.md     # Agent instructions
│   ├── files/
│   └── tools/
│       └── __init__.py
├── servers/
│   └── claude_skills/      # MCP helper modules for Claude Skills
├── mnt/
│   └── skills/             # Local skills consumed by Claude Skills backend
...
Follow this folder structure when creating or modifying any files. When adding new agents, mirror the
layout of the existing `skill_creator/` folder.
