"""Claude Skills MCP Server Configuration"""
...
def get_server() -> MCPServerStdio:
    """Get the Claude Skills MCP server instance (singleton)"""
    global _server
    if _server is None:
        _server = MCPServerStdio(
            ...
            params={
                "command": "uvx",
                "args": ["--python", "3.12", "claude-skills-mcp"],
            },
...
async def call_tool(tool_name: str, arguments: dict):
    ...
    # Get the session
    session = getattr(server, "session", None) or getattr(
        server, "_client_session", None
    )
    if not session:
...
# Tool discovery - run with: python ./servers/claude_skills/server.py
            ...
            for tool in tools:
                ...
                if hasattr(tool, "inputSchema") and "properties" in tool.inputSchema:
                    ...
                    for param_name, param_info in schema["properties"].items():
                        ...
                        print(
                            f"    - {param_name}: {param_type} {'(required)' if required else '(optional)'}"
                        )
                        if param_desc:
                            print(f"      {param_desc}")
                print()
